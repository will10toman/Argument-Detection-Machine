IBM Debater(R): Claims and Evidence ACL'14

** Dataset Description **

1,392 labeled claims for 33 different topics, and 1,291 labeled evidence for 350 distinct claims in 12 different topics. 
These data were published by Aharoni et al. in the First Workshop on Argumentation Mining at ACL-2014.

** Licensing **

The datasets are released under the following licensing and copyright terms:
• (c) Copyright Wikipedia (https://en.wikipedia.org/wiki/Wikipedia:Copyrights#Reusers.27_rights_and_obligations)
• (c) Copyright IBM 2014. Released under CC-BY-SA (http://creativecommons.org/licenses/by-sa/3.0/)

** Content **
The dataset includes:
1. Two CSV files containing, for each topic, the claims and evidence that were identified for it in relevant Wikipedia articles. 
2. The original Wikipedia articles - from Wikipedia April 2012 dump - in the form of text files, cleaned from any Wikisyntax or HTML markup. 
 
	The name of each file corresponds to the name in the article column of the published data files with the following changes:

	1. Spaces replaced by underscores
	2. Ampersand (&) replaced by 'and'
	3. Question marks removed
	4. Me'rida_Initiative uses a standard ' character 